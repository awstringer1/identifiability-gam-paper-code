...
Type objective_function<Type>::operator() ()
{
  ...
  Type loglik = 0;
  for (int i=0;i<n;i++) loglik -= y(i)*eta(i) - exp(eta(i));
  ...
  bRprior -= -0.5*Rdub*log(2.0*pi);
  int tmpidx2 = 0;
  for (int j=0;j<p;j++) {
    bRprior -=0.5*rdub(j)*logsmoothing(j);
    for (int l=0;l<r(j);l++) {
      bRprior -= -0.5*smoothprec(j) * bR(tmpidx2)*bR(tmpidx2);
      tmpidx2++;
    }
  }
  ...
  REPORT(loglik);
  REPORT(bRprior);
  Type out = loglik + bRprior;
  REPORT(out);
  
  return out; // Actually minus loglik
}
