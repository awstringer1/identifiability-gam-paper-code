### Simulations: Poisson Data, Optimal Constraints ###
...
truefunc <- function(x) sin(2*pi*x)
# Intercept
alpha <- 1
...
tmbpath <- "~/work/projects/concurvity-gam/code/tmb/poisson_gam" # TODO: change this
...
fit_poisson_gam <- function(smooth,dat,preddat=dat,constr,method=c('BFGS','trust'),...) {
  ## SETUP CONTROL ##
  method <- method[1]
  ...
  condest <- as.numeric(Xpred[ ,-1] %*% condcoefs[-1]) # No intercept
  ...
  list(
    ...
    sd = sqrt(estvar),
    prec = H,
    lambda = exp(opt$solution),
    XZ = XZ,
    PZ = PZ
  )
}

# Function to fit model with specific constraint cc to particular dataset
...
doopt <- function(lst) {
  ...
  Xlist <- lapply(SS,'[[','X')
  X <- Reduce(cbind,Xlist)
  compare <- fit_model_constr(colSums(X),dat,preddat)
...
# Save results
write_csv(simframe,file.path(resultspath,"poisson-optconstraint-sims.csv"))
